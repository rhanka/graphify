...
describe("release configuration", () => {
  it("tracks the 0.9.2 patch release across package metadata and changelog", () => {
    ...
    expect(lock.version).toBe("0.9.2");
    expect(lock.packages?.[""]?.version).toBe("0.9.2");
    expect(changelog).toContain("## 0.9.2 (2026-05-16)");
  });
