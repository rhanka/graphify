...
describe("release configuration", () => {
  it("tracks the 0.11.1 release across package metadata and changelog", () => {
    ...
    expect(lock.version).toBe("0.11.1");
    expect(lock.packages?.[""]?.version).toBe("0.11.1");
    expect(changelog).toContain("## 0.11.1 (2026-06-12)");
  });
