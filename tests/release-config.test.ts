...
describe("release configuration", () => {
  it("tracks the 0.11.0 release across package metadata and changelog", () => {
    ...
    expect(lock.version).toBe("0.11.0");
    expect(lock.packages?.[""]?.version).toBe("0.11.0");
    expect(changelog).toContain("## 0.11.0 (2026-06-12)");
  });
