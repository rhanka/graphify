...
In Codex, prefer `$graphify ...` as the explicit invocation. Do not rely on `/graphify ...`, which is Claude syntax.

## What graphify is for

graphify is built around Andrej Karpathy's /raw folder workflow: drop anything into a folder - papers, tweets, screenshots, code, notes - and get a structured knowledge graph that shows you what you didn't know was connected.

Three things it does that a one-shot assistant session cannot:
...
- Images (.png/.jpg/.webp) → downloaded, LLM vision extracts on next run
...
This writes a `## graphify` section to the local `AGENTS.md` and registers a `.codex/hooks.json` PreToolUse hook so Codex checks the graph before answering codebase questions and keeps it current after code changes. No manual `$graphify` is needed for routine graph-aware codebase questions in future sessions.
