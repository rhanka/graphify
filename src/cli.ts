...
export async function main(): Promise<void> {
  ...
  program
    ...
    .action(async (updatePath = ".", opts) => {
      ...
      console.log(
        `Re-extracting code files in ${updatePath}` +
        (describe ? " (descriptions on by default; --no-description to skip)..." : " (no LLM needed)..."),
      );
  ...
  program
    ...
    .action(async (opts) => {
      ...
      await rebuildCode(".", false, {
        clearStale,
        // WP11: the automatic git-hook rebuild stays fast and LLM-free; node
        // descriptions are generated by `graphify update` (default-on) instead.
        describe: false,
        scope: scopeSelection.mode,
        scopeSource: scopeSelection.source,
      });
