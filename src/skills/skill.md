...
fs.writeFileSync('graphify-out/GRAPH_REPORT.md', report);
fs.writeFileSync('graphify-out/.graphify_labels.json', JSON.stringify(Object.fromEntries(Object.entries(labels).map(([k, v]) => [String(k), v]))));
...
const extraction = JSON.parse(fs.readFileSync('graphify-out/.graphify_extract.json', 'utf-8'));
const analysis = JSON.parse(fs.readFileSync('graphify-out/.graphify_analysis.json', 'utf-8'));
const labelsRaw = fs.existsSync('graphify-out/.graphify_labels.json') ? JSON.parse(fs.readFileSync('graphify-out/.graphify_labels.json', 'utf-8')) : {};
...
const G = buildFromJson(JSON.parse(fs.readFileSync('graphify-out/.graphify_extract.json', 'utf-8')));
...
fs.writeFileSync('graphify-out/.graphify_incremental.json', JSON.stringify(result));
if (newTotal === 0) {
...
const result = fs.existsSync('graphify-out/.graphify_incremental.json') ? JSON.parse(fs.readFileSync('graphify-out/.graphify_incremental.json', 'utf-8')) : {};
...
// Load new extraction
const newExtraction = JSON.parse(fs.readFileSync('graphify-out/.graphify_extract.json', 'utf-8'));
const GNew = buildFromJson(newExtraction);

// Prune nodes from deleted files
const incremental = JSON.parse(fs.readFileSync('graphify-out/.graphify_incremental.json', 'utf-8'));
...
const oldData = fs.existsSync('graphify-out/.graphify_old.json') ? JSON.parse(fs.readFileSync('graphify-out/.graphify_old.json', 'utf-8')) : null;
const newExtract = JSON.parse(fs.readFileSync('graphify-out/.graphify_extract.json', 'utf-8'));
