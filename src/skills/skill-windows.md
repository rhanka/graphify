...
```powershell
if (-not (Get-Command graphify -ErrorAction SilentlyContinue)) { npm install -g graphifyy 2>&1 | Select-Object -Last 3 }
...
const analysis = fs.existsSync('.graphify_analysis.json')
  ? JSON.parse(fs.readFileSync('.graphify_analysis.json', 'utf-8'))
  : null;
...
fs.writeFileSync('.graphify_detect_semantic.json', JSON.stringify(semanticDetect, null, 2));
fs.writeFileSync('.graphify_transcripts.json', JSON.stringify(transcriptPaths, null, 2));
...
if (codeFiles.length > 0) {
    ...
    fs.writeFileSync('.graphify_ast.json', JSON.stringify(result, null, 2));
    console.log(\`AST: ${result.nodes.length} nodes, ${result.edges.length} edges\`);
} else {
    fs.writeFileSync('.graphify_ast.json', JSON.stringify({nodes:[],edges:[],input_tokens:0,output_tokens:0}));
...
const detect = JSON.parse(fs.readFileSync('.graphify_detect_semantic.json', 'utf-8'));
const allFiles = [
...
if (cachedNodes.length || cachedEdges.length || cachedHyperedges.length) {
    fs.writeFileSync('.graphify_cached.json', JSON.stringify({nodes: cachedNodes, edges: cachedEdges, hyperedges: cachedHyperedges}));
}
fs.writeFileSync('.graphify_uncached.txt', uncached.join('\n'));
...
const raw = fs.existsSync('.graphify_semantic_new.json') ? JSON.parse(fs.readFileSync('.graphify_semantic_new.json', 'utf-8')) : {nodes:[],edges:[],hyperedges:[]};
...
const cached = fs.existsSync('.graphify_cached.json') ? JSON.parse(fs.readFileSync('.graphify_cached.json', 'utf-8')) : {nodes:[],edges:[],hyperedges:[]};
const raw = fs.existsSync('.graphify_semantic_new.json') ? JSON.parse(fs.readFileSync('.graphify_semantic_new.json', 'utf-8')) : {nodes:[],edges:[],hyperedges:[]};
...
fs.writeFileSync('.graphify_semantic.json', JSON.stringify(merged, null, 2));
...
"
```
Clean up temp files: `Remove-Item -ErrorAction SilentlyContinue .graphify_cached.json, .graphify_uncached.txt, .graphify_semantic_new.json, .graphify_detect_semantic.json, .graphify_transcripts.json`
...
const ast = JSON.parse(fs.readFileSync('.graphify_ast.json', 'utf-8'));
const sem = JSON.parse(fs.readFileSync('.graphify_semantic.json', 'utf-8'));
...
fs.writeFileSync('.graphify_extract.json', JSON.stringify(merged, null, 2));
...
const obsidianDir = 'OBSIDIAN_DIR';  // replace with --obsidian-dir value, or 'graphify-out/obsidian' if not given
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "npx",
      "args": ["graphify", "serve", "/absolute/path/to/graphify-out/graph.json"]
    }
  }
}
```
...
const detect = JSON.parse(fs.readFileSync('.graphify_incremental.json', 'utf-8'));
const analysis = fs.existsSync('.graphify_analysis.json')
  ? JSON.parse(fs.readFileSync('.graphify_analysis.json', 'utf-8'))
  : null;
...
fs.writeFileSync('.graphify_incremental_semantic.json', JSON.stringify(semanticDetect, null, 2));
fs.writeFileSync('.graphify_transcripts.json', JSON.stringify(transcriptPaths, null, 2));
console.log(\`Transcribed ${transcriptPaths.length} video file(s) -> treating as docs\`);
"
```
...
1. **Upgrade graphify**: `npm install -g graphifyy`
