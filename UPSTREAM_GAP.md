...
- Current TypeScript package target: `graphifyy@0.7.10` (publish pending release gate)
...
`graphifyy@0.9.4` is numerically ahead of upstream stable `graphify@0.8.11`, but the version number is still **not** a blanket parity claim against `v8` / `v0.8.x`. The `0.9.x` line is driven by TypeScript product deltas plus audited upstream carry-forward. This branch advances the closest audited parity point to upstream `v0.8.9` by porting `2aaa216`; the remaining `f7160c8` and `2d783e5` rows above still define the functional gap before any stronger `v0.8.11` parity claim.
