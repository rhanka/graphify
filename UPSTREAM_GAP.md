...
| --- | --- | --- | --- | --- |
| Python `v0.7.5`..`v0.7.10` | Incremental update and dedup | semantic cache/build-merge/manifest, entity dedup design, MinHash/Jaro-Winkler strategy, reversed call-edge update fix, community label persistence | `partial` | Reversed call-edge preservation is ported for `buildMerge`; continue semantic cache, manifest, dedup, and community-label audit before heavier dedup parity. |
| Python `v0.7.5`..`v0.7.10` | Parser and language surface | Markdown structural extraction, `.md/.mdx` update path, TS interface/enum/type-alias/const/new_expression, TSX JSX parser, CommonJS require, Groovy/Spock, Luau, Quarto `.qmd`, SQL ALTER FK and schema-qualified names | `partial` | SQL ALTER FK, schema-qualified table names, CommonJS `require()` module/symbol edges, and TS/TSX interface/type/enum/const/new-expression/JSX call extraction are ported through the TypeScript extractor; continue Markdown, Quarto and selected no-Python language audit. |
...
| Python `v0.7.5`..`v0.7.10` | MCP/query/install/security | MCP resources, edges/links loader fix, Kimi platform, positional install platforms, uninstall-all command, skill YAML descriptions, subprocess hardening | `partial` | Top-level `graphify uninstall`, missing-skill repair warning, and atomic skill writes are ported; continue query/MCP/install-argument/security audit. |
