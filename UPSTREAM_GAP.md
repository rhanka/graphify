...
| Upstream ref | Upstream scope | TS status | Plan lot | Catch-up action |
| --- | --- | --- | --- | --- |
| `v0.6.0` / `17fb524` | SQL AST extraction, YAML indexing, roll-up bug fixes from `0.5.6`/`0.5.7` | `partial` | Structured inputs, query precision, and inventory semantics | YAML corpus detection is now covered and `.sql` files now enter the code surface, but SQL AST extraction parity is still open. |
| `v0.6.1` / `2dc759a` | exact gitignore semantics, anchored patterns, hermetic non-VCS scan | `needs-review` | Structured inputs, query precision, and inventory semantics | Compare upstream ignore semantics with current `.graphifyignore` and input-scope behavior; add regression tests before closure. |
| `v0.6.2` / `be83a8c` | exact-match query ordering, smarter content-hash `update`, R support, shebang detection, Kimi/license cleanup | `partial` / `n/a` | Structured inputs, query precision, and inventory semantics | Exact-match query scoring, `.r` support, and extensionless shebang-script detection are now covered; content-hash-aware `update` still needs its own audit/implementation. |
| `v0.6.3` / `a4149df` | preserve semantic nodes on incremental rebuild, detached hooks, common-name suppression, `cluster-only` crash guard | `needs-review` | Incremental rebuild reliability, hooks, and platform surface | Compare update/hook/analysis flows and prove behavior with update/watch/runtime tests. |
| `v0.6.4` / `a61b25c` | cross-platform Codex hook check on Windows | `needs-review` | Incremental rebuild reliability, hooks, and platform surface | Audit the current Codex hook installer/runtime against upstream shell-agnostic behavior. |
| `v0.6.5` / `d40e1c0` | Codex Windows hook, Kotlin call edges, `update --force`, community checkbox UI | `needs-review` | Incremental rebuild reliability, hooks, and platform surface | Audit hook portability, Kotlin surface, guarded force-update behavior, and HTML control parity. |
| `v0.6.6` / `517f3c8` | Pi platform install, wiki stale clearing, Windows-safe wiki filenames | `needs-review` / `deferred` | Incremental rebuild reliability, hooks, and platform surface | Decide whether Pi support belongs in the TypeScript platform matrix; audit wiki regeneration and filename sanitization either way. |
| `v0.6.7` / `e484282` | `graphify tree`, token-aware chunking, MCP context filters, dynamic `import()`, safe semantic-cache file checks | `needs-review` | Visualization, ignore semantics, and portable output routing | Audit the tree/export surface, chunking policy, context filtering, JS import extraction, and directory-safe cache handling. |
| `v0.6.8` / `d753413` | `.graphifyignore` negation patterns, Antigravity frontmatter, Gemini/Codex hook fixes, thin-community omission | `needs-review` | Visualization, ignore semantics, and portable output routing | Audit ignore negation semantics, platform hook installers, and report community filtering. |
| `v0.6.9` / `f81e3bc` | slash-normalized `source_file`, cohesion re-splitting, VS Code Copilot instruction contract, `GRAPHIFY_OUT`, Antigravity reinstall | `needs-review` / `intentional-delta` | Visualization, ignore semantics, and portable output routing | Audit portability/report clustering changes; explicitly decide whether `GRAPHIFY_OUT` is compatible with the TypeScript `.graphify/` contract or remains a deliberate delta. |
| `v0.7.0` / `f81e3bc` | merge driver for `graph.json`, deterministic community IDs, content-only cache on renames, freshness signal, mixed code/doc handling | `needs-review` | Multi-developer graph lifecycle | Audit multi-dev workflow support against existing `.graphify` lifecycle and portable artifact model before claiming parity. |
| `v0.7.1` / `f81e3bc` | Obsidian tag sanitization, extended `tsconfig` alias resolution, Svelte template dynamic imports, recursion safety | `needs-review` | Parser robustness, export surface, and headless extraction | Audit wiki/export sanitization, TS config resolution, Svelte import extraction, and deep-AST robustness. |
| `v0.7.2` / `f81e3bc` | Fortran support, export CLI subcommands, skill-size reduction, large-graph aggregation | `needs-review` / `deferred` | Parser robustness, export surface, and headless extraction | Audit language and export surfaces; treat skill-size reduction as non-blocking unless it changes runtime behavior or install limits. |
...
| `v0.7.4` / `f81e3bc` | JSONC `tsconfig` parsing and aliased Svelte dynamic-import fixes | `needs-review` | Parser robustness, export surface, and headless extraction | Close parser/import parity and use this row as the final gate before publishing `0.7.4`. |
