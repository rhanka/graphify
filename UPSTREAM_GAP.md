...
| `v0.6.7` / `e484282` | `graphify tree`, token-aware chunking, MCP context filters, dynamic `import()`, safe semantic-cache file checks | `partial` | Visualization, ignore semantics, and portable output routing | `graphify tree` is now covered by `tests/cli-runtime.test.ts` (`supports tree for compact graph traversal output`), local JS/TS `import()` extraction is covered by `tests/language-surface.test.ts` (`resolves local dynamic imports as imports_from edges`), and directory-safe semantic-cache writes are covered by `tests/cache.test.ts` (`skips directory source_file entries when saving semantic cache`) plus the `fileHash requires a file` guard in `src/cache.ts`. Token-aware chunking policy and MCP context-filter parity are still open. |
| `v0.6.8` / `d753413` | `.graphifyignore` negation patterns, Antigravity frontmatter, Gemini/Codex hook fixes, thin-community omission | `partial` | Visualization, ignore semantics, and portable output routing | `.graphifyignore` negation semantics are now covered by `tests/detect.test.ts` (`supports .graphifyignore negation patterns`), and thin-community omission is already covered by `tests/report.test.ts` (`does not list empty communities in the report`). Codex hook portability was closed earlier via `graphify hook-check`; Antigravity workflow frontmatter/install polish and Gemini parity still need explicit audit in this lot. |
