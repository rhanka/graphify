...
| `v0.6.3` / `a4149df` | preserve semantic nodes on incremental rebuild, detached hooks, common-name suppression, `cluster-only` crash guard | `partial` | Incremental rebuild reliability, hooks, and platform surface | Code-only rebuilds now preserve existing semantic nodes and their surviving edges, covered by `tests/cli-runtime.test.ts` (`preserves existing semantic nodes during code-only update rebuilds`). Detached hooks, common-name suppression, and the remaining `cluster-only` parity audit stay open. |
...
| `v0.6.5` / `d40e1c0` | Codex Windows hook, Kotlin call edges, `update --force`, community checkbox UI | `partial` | Incremental rebuild reliability, hooks, and platform surface | `update --force` is now covered by `tests/export-json.test.ts` and `tests/cli-runtime.test.ts`: guarded rebuilds fail cleanly when `graph.json` would shrink, and `--force` explicitly overrides that guard. Hook portability, Kotlin call edges, and HTML checkbox controls remain open. |
