...
| `v0.4.7` / `c713cf8` | watch edge-key fix, OpenClaw path correction, Blade support, WSL MCP docs | `partial` | Lot 1, Lot 5, Lot 8 | Blade classification/extraction is covered by Lot 1. Verify OpenClaw path support and WSL MCP docs in platform/docs lots. |
...
| `v0.4.9` / `7c81c1b` | PHP extractor improvements, Dart, diacritic search, Hermes, fixes | `partial` | Lot 1, Lot 4, Lot 5 | Dart and Blade/PHP surface coverage are covered by Lot 1. Hermes platform support and diacritic search/normalization remain. |
...
| `v0.4.13` / `79a9200` | Verilog/SystemVerilog, HiDPI hyperedge fix, null label guards, generated instructions use `graphify update .` | `partial` | Lot 1, Lot 4, Lot 7 | Verilog/SystemVerilog detection/extraction is covered by Lot 1. Hyperedge/null-label behavior remains for export/report guard verification. |
...
| `v0.4.15` / `7ab62fd` | VS Code Copilot Chat, OpenCode/Gemini Windows fixes, `.mjs`/`.ejs`, macOS watch behavior, `god_nodes` degree rename | `partial` | Lot 1, Lot 5, Lot 8 | MJS/EJS input support is covered by Lot 1. VS Code Copilot Chat platform and `edges`/`degree` compatibility remain. |
...
| `v0.4.20` / `69a0cfc`, `36fa62a` | JS/MJS import path normalization; CLI update/cluster-only emits `graph.html` | `covered` | Lot 1, Lot 3, Lot 7 | JS import normalization, `.mjs` dispatch, and best-effort HTML output are covered by TS runtime tests. |
