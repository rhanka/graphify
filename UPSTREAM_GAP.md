...
`graphifyy@0.9.2` is numerically ahead of upstream stable `graphify@0.8.8`, but the version number is **not** a parity claim against `v8` / `v0.8.x`. The `0.9.x` line is driven by TypeScript product deltas and install-contract fixes, not mechanical Python tag mirroring. Track F closed the upstream #891 query-first guidance gap in `0.9.2`; the remaining F-P2/F-P3/F-M1 rows above still define the functional gap before any stronger `v0.8.x` parity claim.
