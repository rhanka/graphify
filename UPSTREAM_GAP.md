...
| `v0.4.7` / `c713cf8` | watch edge-key fix, OpenClaw path correction, Blade support, WSL MCP docs | `partial` | Lot 1, Lot 5, Lot 7 | Blade classification/extraction is covered by Lot 1. Platform preview/install coverage now covers the v4 platform surface; WSL MCP docs remain in the documentation lot. |
...
| `v0.4.10` / `e441454` | Cursor install crash, OpenCode uninstall scoping, Codex `wait_agent`, Dart/Hermes, PHP features, `path`/`explain`/`add`/`watch`/`update`/`cluster-only` bare commands | `covered` | Lot 1, Lot 5, Lot 6 | CLI surface is ahead of upstream; Dart/Hermes/Blade platform and language coverage are verified. Bare command runtime audit remains as regression coverage in Lot 6. |
...
| `v0.4.19` / `2c5d3a5`, `76d1203` | normalized IDs in build, cross-file calls Go/Rust/Zig/PowerShell/Elixir, resolved cache path, `core.hooksPath`, Kiro YAML, team workflow docs | `partial` | Lot 5, Lot 7 | `core.hooksPath`, cache path, cross-file call behavior, and Kiro quoted YAML are covered; team workflow/docs remain in the documentation lot. |
