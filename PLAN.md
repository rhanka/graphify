...
**Status on 2026-06-08:** `main` is at `@sentropic/graphify@0.10.0` (package rename via PR #79, tag `v0.10.0`, published 2026-05-26; previously `graphifyy`). The Track F `0.8.16` and `0.8.18` drift cycles are both **closed and released** (`0.9.7` via PRs #59/#62-#69, `0.9.8` via PRs #74-#77). Since then, two further parity batches are **merged on `main` but unreleased on npm**: F-0819-P1/P2 (PRs #87/#88 — Lua `require()` resolution `#1075`, arrow-fn/markdown phantom god-nodes `#1077`, manifest prune path matching `#1007`, deterministic edge sort `#1010`) and F-0831-P1-security (PR #93 — office/PDF zip-bomb cap, `OLLAMA_BASE_URL` SSRF hardening, Fortran cpp-path non-regression guard). Residual Track F items (F-0816-M3 bash hardening: parked; `#996` semantic-contexts: deferred pending scope decision; SCIP helpers: deferred; F-Opt-LLM-Triage: closed as already-covered at 0.9.7) are not release blockers. The next parity number waits on the formal bilan of the `v0.8.18..v0.8.31` window (`upstream/v8` fetched locally through `v0.8.31`/`7a588fb`; the F-0819/F-0831-P1 lots above are early ports from that window, to be reconciled by the bilan). Earlier history for context: PR #22 merged the Python `0.7.10` checkpoint; Track D closed the `0.7.11..0.7.19` drift band at `0.7.19`. Product work continues to run in parallel with upstream drift via independent lanes.
...
| **F** Upstream parity weekly | F-0818 publié en `0.9.8` ; rename `@sentropic/graphify@0.10.0` (PR #79) publié ; F-0819-P1/P2 (PRs #87/#88) et F-0831-P1-security (PR #93) mergés sur `main`, non publiés. Prochaine action : bilan formel de la fenêtre `v0.8.18..v0.8.31` (upstream local jusqu'à `v0.8.31`/`7a588fb`), réconciliant les ports 0819/0831 déjà mergés. | **DECISION user** — (1) scope du `#996` semantic-contexts (présentation détaillée transmise 2026-06-08) ; (2) véhicule de release npm des lots 0819/0831 + studio mergés non publiés (patch `0.10.1` vs minor `0.11.0`). |

## Detailed Track Plans

### Track A: Descriptions — DONE (archived 2026-05-16)

A1 schema + A1 render + assistant + direct + cache invalidation + ontology pages + A2 batch scaffold + **A3 mesh bridge** all landed. Bridge `@sentropic/llm-mesh` proven E2E via `tests/wiki-description-generation.test.ts > "A-final"` (PR #33). Released as `graphifyy@0.9.1`. See `CHANGELOG.md` 0.9.1 and earlier 0.7.x entries for details.

### Track B: Reconciliation — DONE (archived 2026-05-16)

Patch core + candidate queue + read-only MCP/HTTP/CLI + write-enabled studio + decision-log preview + public-pack live walk all landed. Svelte reconciliation studio remains blocked on `../sent-tech-design-system` tokens (external dep); static HTML viewer covers the user surface in the meantime. See `CHANGELOG.md` 0.7.19-rc.1 entry.

### Track D: Upstream Python `0.7.11..0.7.19` Drift — DONE (archived 2026-05-16)

M0..M5 lots all landed. `.astro` extractor (`6a7de56`), watch `.rebuild.lock` (`a641d97`), `--no-cluster` + topology short-circuit (`d05bb09`), `--backend claude-cli` (`61957e6`). npm promoted to `0.7.19` stable. See `UPSTREAM_GAP.md` drift matrices.

### Track C: CRG Additive UX And Review Precision (Overall **9/13 = 69 %**)
...
**Bilan #2 refresh (2026-05-18).** Current branch ports upstream `2aaa216` (`v0.8.9`) so npm `package.json` dependency-block keys no longer surface as god nodes. This follow-up branch closes `f7160c8` by making Rust `Type::method()` resolution stay on matching local impl methods instead of bare last-segment matches; the next recommended `upstream/v8` follow-up is now `2d783e5` (cohesion unrounding audit plus hook/detect follow-ups). `F-Opt` remains deferred until upstream ships a stable `v2.0.0` tag.
...
- [x] **Lot F-P1 — Windows / hook stability** (~3-4 days). Target upstream commits:
  ...
  Cleans up the Windows test-run footprint and prevents `.graphify_temp/` style leakage into the working tree. Covered by `tests/hooks.test.ts`, `tests/install-preview.test.ts`, `tests/platform-v4-integration.test.ts`, `tests/skills.test.ts`, and `tests/cli-runtime.test.ts`.
- [x] **Lot F-P2 — Graph correctness + MCP watch** (~2-3 days). Target upstream commits:
  ...
  Plus MCP hot-reload of `.graphify/graph.json` if upstream now publishes a fs-watcher hook; otherwise downgrade to MCP file-mtime check. Covered by `tests/detect.test.ts`, `tests/build-merge.test.ts`, `tests/analyze.test.ts`, and `tests/serve.test.ts`.
...
- [x] F-M1 infra impl (SQL FK/trigger extraction + community label normalisation).
...
- [x] **Lot F-0818-P1 — Call-resolution correctness** (PR #74, released 0.9.8). Target upstream commit:
...
- [x] **Lot F-0818-M1 — Hook/skill/worker hygiene** (PR #75, released 0.9.8; per-worker isolation #943 already-covered). Target upstream commit:
...
- [x] **Lot F-0818-M2 — Backup rate-limit + community sidecar fallback** (PR #76, released 0.9.8; community reconstruction #1001 already-covered by `communitiesFromCliGraph`). Target upstream commits:
...
- [x] **Lot F-0818-M3 — Query expansion + Unicode vocab regex** (PR #77, released 0.9.8; skill-guidance port). Target upstream commits:
