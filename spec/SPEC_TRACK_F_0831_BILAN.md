...
| P3 | `ad3f3b2` | — | Harden XML parsing against billion-laughs DoS (`extract_csproj`/`extract_lpk`) | new XML guard, paired with the `.NET` extractor port | F-0831-P2 (paired F-0819-M) | **ported F-0819-M.** `_projectXmlIsSafe()` in `src/extract.ts` pre-screens raw bytes for `<!DOCTYPE` / `<!ENTITY` before any parse; `_PROJECT_XML_MAX_BYTES` (2 MiB) caps input size. Guard applied in `extractCsproj` (all .csproj/.fsproj/.vbproj/.props/.targets). Tests: `tests/extract-dotnet.test.ts` "rejects a DOCTYPE declaration", "rejects an ENTITY-only declaration", "rejects input larger than 2 MiB", "accepts valid XML without DOCTYPE/ENTITY" — all green. Commit: `feat(track-f): .NET project-file extractor with paired XML billion-laughs guard (F-0819-M)` on `feat/track-f-0819m-dotnet`. |
| P4 | `e3993e4` | F1 | Validate custom-provider `base_url`, gate project-local providers | `src/security.ts:168-226` (`providerBaseUrlOk`), `src/provider-registry.ts` (`loadCustomProviders` + GRAPHIFY_ALLOW_LOCAL_PROVIDERS gate), `tests/provider-base-url.test.ts` (21 tests) | **ported** — F-0831-P2a | **ported** by commit `fix(track-f): validate LLM provider base_url against SSRF/metadata egress (F-0831-P2a, e3993e4)` on branch `feat/track-f-0831-p2a-baseurl`. Paired M14 registry (`a9d6be6`) minimal loader also ported in same commit (never ship registry without validation). Already-covered in TS before this commit: `validateOllamaBaseUrl` (PR #93, b2e71c8, `src/security.ts:116-167`) covers the OLLAMA path — NOT duplicated here. This port extends scheme + plaintext-http validation and project-local gating to the custom-provider surface. |
| P5 | `0fdfded` | — | Harden hook interpreter detection against injection and unquoted exec | `src/hooks.ts` (hook script generation), `tests/hooks.test.ts` | F-0831-P2b ✅ already-covered | **already-covered — architecture differs, no injection vector.** Python embeds `sys.executable` (a filesystem path) at install time via `__PINNED_PYTHON__` placeholder; TS uses runtime detection (`command -v graphify` → `"graphify"` literal; else `"npx graphify"` literal). No filesystem-sourced interpreter path is ever templated into the hook scripts. The two `GRAPHIFY_CMD` values are hard-coded string literals in the TS template (`src/hooks.ts:55,59`), not derived from `process.execPath` or any external input — no allowlist sanitization needed. The upstream `nohup "$GRAPHIFY_PYTHON"` quoting fix maps to `> "$GRAPHIFY_LOG"` already being double-quoted in TS (`src/hooks.ts:80`). **Proof**: 3 non-regression tests added in `tests/hooks.test.ts` (describe block `hook injection safety (0fdfded non-regression)`) — all green. |
...
| # | Upstream SHA | Issue | Subject | Proposed TS target | Lot | Rationale / Evidence |
|---|---|---|---|---|---|---|
| M1 | `8bcfffd` | — | .NET project file support (`.sln`, `.csproj`, `.fsproj`, `.vbproj`, `.props`, `.targets`) | `src/detect.ts`, `src/extract.ts`, tests | F-0819-M | **ported F-0819-M.** `extractSln` + `extractCsproj` (sync, regex-backed, no XML parser) added to `src/extract.ts`; dispatched for `.sln/.csproj/.fsproj/.vbproj/.props/.targets`. `CODE_EXTENSIONS` + `_EXTENSIONS` updated in `src/detect.ts` and `src/extract.ts`. Fixtures: `tests/fixtures/sample.sln` + `tests/fixtures/sample.csproj`. 31 tests in `tests/extract-dotnet.test.ts` — all green. Paired with P3 XML-DoS guard in the same commit. Note: `.razor`/`.cshtml` (Blazor) deferred (no tree-sitter Razor grammar available in TS); `.props`/`.targets` extended vs upstream as they share the MSBuild XML schema. |
| M2 | `9abaa77` | #1028 | Apply `remap_communities_to_previous` in cluster-only path | `src/cluster.ts`, `src/cli.ts:3326` (cluster-only), `src/community-labels.ts` | F-0820-0827 | **PORTED F-0820-0827.** `remapCommunitiesToPrevious()` added to `src/cluster.ts` (greedy overlap matching); wired into `cluster-only` CLI path (`src/cli.ts`): reads `node.community` from existing `graph.json`, builds `previousNodeCommunity` map, calls `remapCommunitiesToPrevious` if non-empty. 5 unit tests + 1 integration test in `tests/cluster-remap-to-previous.test.ts`. Pairs with M19. |
...
| M4 | `baaab5f` | #999 | Dart child node IDs via `_file_stem` (no machine-specific absolute paths in `graph.json`) | `src/extract.ts` (Dart), tests | F-0820-0827 | **already-covered.** TS routes `.dart` through `extractRegexBackedCode` which uses `qualifiedFileStem(filePath, rootDir)` for all symbol IDs — stem-based by construction, never embeds absolute paths. Non-regression test: `tests/extract-dart-stem-ids.test.ts` (`src/extract.ts:2482-2495` + `4790`). |
| M5 | `80301a0` (part) | #916 | Builtin god-node filter | `src/analyze.ts`, tests | F-0820-0827 | **PORTED F-0820-0827.** `LANGUAGE_BUILTIN_GLOBALS` set exported from `src/extract.ts`; `emitCallByName` bails early when callee is in the set; covers JS/TS and Python builtins. Tests: `tests/extract-builtin-godnode-filter.test.ts` (5 tests). |
| M6 | `9f73400` | #1047, #1046, #1050 | Memory-dir gitignore leak; Pass-2 dedup cross-file identical merge; decorated method node ID mismatch | `src/detect.ts:685-689` (memory dir), dedup pass, Python decorator IDs | F-0820-0827 | **PORTED F-0820-0827 (M6a/M6b/M6c).** M6a: `decorated_definition` transparent wrapper added to `walk()` in `src/extract.ts` — decorated Python methods now inherit `parentClassNid`. Tests: `tests/extract-python-decorated-method.test.ts` (4 tests). M6b (dedup): already-covered — `src/extract.ts` uses `seenEdges` Set for cross-file dedup; test `tests/extract-dedup-cross-file.test.ts` proves it. M6c (memory-dir): already-covered — `SKIP_DIRS` constant in `src/detect.ts` already excludes `.memory/` and the detect function reads only from the classified file lists; test `tests/detect-memory-dir-skip.test.ts` proves it. |
...
| M9 | `379d35e` | #1063 | claude-cli: eliminate hollow-response loop from system-prompt conflict | TS claude CLI execution port (`spec/SPEC_LLM_EXECUTION_PORTS.md` surface) | F-0820-0827 | **n-a (TS).** TS fork uses the Anthropic SDK directly — no `_call_claude_cli` subprocess, no `--append-system-prompt` flag, no hollow-response bisection loop. Nothing to port. |
| M10 | `f0badd9` (part) | — | Post-commit hook silent drop on rapid commits | `src/hooks.ts`, `tests/hooks.test.ts` | F-0820-0827 | **n-a (TS) — both halves.** WinError-2 fix is Python `subprocess` specific. Rapid-commit race fix targets `graphify/watch.py`'s file-lock queue mechanism (`_queue_pending`, `_PENDING_FILENAME`) — TS hooks background `hook-rebuild` via `nohup ... &` without a queue; no equivalent surface exists. |
| M11 | `9985940` | #1079 | Antigravity global install path `~/.gemini/config/skills/` + uninstall symmetry | `src/cli.ts` (antigravity platform map) | F-0820-0827 | **PORTED F-0820-0827.** `PLATFORM_CONFIG.antigravity.skill_dst` changed to `.gemini/config/skills/graphify/SKILL.md`; `project_skill_dst: .agents/skills/graphify/SKILL.md` added to keep project scope separate; `PLATFORM_CONFIG` exported. ANTIGRAVITY_WORKFLOW text updated. Tests: `tests/install-antigravity-platforms.test.ts` (M11 test), `tests/platform-v4-integration.test.ts` + `tests/install-preview.test.ts` updated. |
| M12 | `9a298c5` | — | Write antigravity rules and workflows on project-scoped install | `src/cli.ts` (project-scope path, F-0816-P1 lineage) | F-0820-0827 | **PORTED F-0820-0827.** `projectInstall("antigravity")` now calls `_antigravityWriteRulesWorkflows()` in addition to `writeProjectSkill`; antigravity extracted from the `["copilot","pi","kimi"]` branch. Tests: `tests/install-antigravity-platforms.test.ts` (3 M12 tests). |
| M13 | `006e159` | #1086 | `extract_files_direct` backend default: auto-detect instead of kimi | `src/llm-execution.ts` (provider default) | F-0820-0827 | **n-a (TS).** TS has no `extract_files_direct(backend="kimi")` function — `kimi` in TS is a platform/IDE install target, not an LLM provider in `DIRECT_LLM_PROVIDERS`. The TS LLM path has no "default kimi" to fix. |
...
| M15 | `cca13aa` | #1087 | Anchored gitignore pattern leaking basename match into subtree | `src/detect.ts:524-546` (`matchesIgnorePattern`) | F-0820-0827 | **already-covered (TS).** TS `matchesIgnorePattern` has an `!anchored` guard: bare `basename.startsWith("/")` test gates the anchored-path branch, so an anchored pattern like `/inbox/` is only matched against the full path (not the basename alone). The Python bug was in Python's custom impl; TS didn't replicate it. Non-regression tests: `tests/detect-anchored-gitignore.test.ts` (3 tests — `/inbox/` must not match `src/inbox/`, `/generated` must not match `src/generated`, unanchored `inbox/` must match `src/inbox/`). |
...
| M17 | `8db19d6` | — | Sort file list lexicographically in `detect()` for deterministic graph output | `src/detect.ts`, `tests/detect.test.ts` | F-0820-0827 | **PORTED F-0820-0827.** Added `allFiles.sort()` before the classify loop + per-FileType `list.sort()` after (`src/detect.ts`). Tests: `tests/detect-file-list-sort.test.ts` (3 tests — lexicographic order, cross-dir order, per-type sort). |
| M18 | `c898dc6` | #1033 | Match AST file-level node IDs to the skill.md spec | `src/extract.ts` node-ID scheme | F-0820-0827 | **PORTED F-0820-0827.** Bug confirmed: `remapFileNodeIds` was mapping nodes with the correct spec-form ID (`qualifiedLegacyId = auth_session`) to the extension-bearing `portableId` (`auth_session_py`). Fix: renamed `portableId` → `canonicalId = _makeId(qualifiedFileStem(...))` (same as `qualifiedLegacyId`); remap now preserves spec-compliant IDs. Updated `tests/language-surface.test.ts` (6 assertions). New tests: `tests/extract-file-node-id-spec.test.ts` (3 tests). |
| M19 | `f5f3a1c` | #1090 | Stabilize community IDs so identical groupings get identical labels | `src/cluster.ts`, `src/community-labels.ts` | F-0820-0827 | **PORTED F-0820-0827.** Bug confirmed: `secondPass.sort()` tiebreak used `a.join("\0")` on unsorted node lists — non-deterministic when partitioner returns same community in different node order. Fix: changed to `[...a].sort().join("\0")` (mirrors Python `tuple(sorted(nodes))`). Tests: `tests/cluster-community-stability.test.ts` (2 tests — stability across enumeration orders, deterministic CID for lex-smaller community). Pairs with M2. |
| M20 | `ad0c8c0` | #1096 | Relativize symbol node IDs for root-level files to match spec | `src/extract.ts` node-ID scheme | F-0820-0827 | **already-covered.** TS extractors use `qualifiedFileStem(filePath, rootDir)` which for root-level files returns the bare stem (since `resolve(parentDir) === resolve(rootDir)` → early-return). Symbol IDs are `_makeId(stem, symbolName)` = `setup_run` — no absolute dir prefix by construction. Non-regression test: `tests/extract-root-symbol-ids.test.ts`. |
| M21 | `88a8e3b` | #1095 | Capture TS interface-extends and same-file class heritage | `src/extract.ts` (TS/JS heritage edges), tests | F-0820-0827 | **PORTED F-0820-0827.** Bug confirmed: TS `interface A extends B` used `extends_type_clause` node (not `class_heritage`), so no inherits edge was emitted; same-file class heritage wasn't resolved via `seenIds`. Fix: added `extends_type_clause` branch in the TS/JS class handler; `emitHeritage` helper resolves against `seenIds` directly before falling back to a stub. Tests: `tests/extract-ts-interface-extends.test.ts` (4 tests — interface extends, multiple bases, class extends same-file, class implements). |
| M22 | `c8b329d` | #1097 | Auto-name communities with the configured backend in standalone CLI | `src/community-labeling.ts` (new), `src/cli.ts` (`graphify label` subcommand), `tests/community-labeling.test.ts` | F-0820-0827 | **ported (v1, feat/track-f-0820-labeling).** New `src/community-labeling.ts`: `buildLabelingPromptLines`, `parseLabelResponse`, `detectLabelingBackend`, `labelCommunities` (injectable `callLlm` for tests), `generateCommunityLabels` (graceful degradation, never throws). New `graphify label [path] [--backend X] [--model Y]` CLI subcommand: re-clusters, calls LLM for community names, persists `.graphify_labels.json`, regenerates report/graph/HTML. Opt-in: no network call without a configured API key. 31 tests, no new deps. v1 parity with upstream Python; the auto-label-on-`cluster-only` path (Python's `--no-label` flag) is deferred. |
| M23 | `e35b0ac` | — | Remove the claude skill tree on uninstall, not just the CLAUDE.md section | `src/cli.ts` (uninstall), tests | F-0820-0827 | **PORTED F-0820-0827.** `claudeUninstall(dir, { skipSkillTree: false })` now calls `uninstallSkill("claude")` to remove `~/.claude/skills/graphify/`; `uninstallAll` passes `skipSkillTree: true` to avoid double-removal. Tests: `tests/install-antigravity-platforms.test.ts` (M23 test). |
| M24 | `5cc7ec8` | — | Close the Read-tool graph bypass with a Read/Glob PreToolUse hook | `src/cli.ts:509-680` (PreToolUse registration) | F-0820-0827 | **PORTED F-0820-0827.** `READ_SETTINGS_HOOK` with `matcher: "Read|Glob"` added; `isGraphifyClaudeHook` recognises `"Read|Glob"`; `installClaudeHook` pushes both hooks; idempotent dedup on re-install. Tests: `tests/install-antigravity-platforms.test.ts` (3 M24 tests); `tests/claude-integration.test.ts` updated to expect ≥2 graphify hooks + `"Read|Glob"` matcher. |
...
| M26 | `75c4de7` | #1112 | Honour `GRAPHIFY_API_TIMEOUT` in claude-cli and Anthropic SDK backends | `src/llm-execution.ts` (+ CLI runner) | F-0820-0827 | **n-a (TS).** The Python fix patches `subprocess.run(timeout=...)` in `_call_claude_cli` and `_call_llm`. TS fork has no claude-subprocess backend — LLM calls go through the Anthropic/OpenAI/Gemini SDKs which manage their own timeout via `fetch` or SDK options. Post-`v0.8.31` commit included in window scope. |
