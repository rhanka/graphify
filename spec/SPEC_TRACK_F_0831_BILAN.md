...
|---|---|---|---|---|---|---|
| P1 | `80301a0` (part) | #1040 | OpenCode project path fix | `src/cli.ts:388-567` (opencode install map + plugin/config entries) | F-0831-P2 | TS ships an OpenCode install surface; wrong project path is user-facing breakage. Sub-rows of `80301a0` continue under M / covered / defer below. |
| P2 | `80301a0` (part) | #1042 | `.svh` Verilog extension | `src/detect.ts:24`, `src/extract.ts:4545` | F-0831-P2 | TS already handles `.v`/`.sv` via `extractRegexBackedCode`; add `.svh` (SystemVerilog headers) to both tables — small, zero-risk. |
