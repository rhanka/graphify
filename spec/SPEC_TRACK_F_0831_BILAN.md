...
| M22 | `c8b329d` | #1097 | Auto-name communities with the configured backend in standalone CLI | `src/cluster.ts` / `src/community-labels.ts` + LLM naming path | F-0820-0827 | **verified-not-covered; deferred to next lot.** TS `cluster-only` path calls `resolveCommunityLabels()` which only reads existing labels or falls back to `"Community <id>"` — no LLM naming call. Python `c8b329d` adds a whole new `graphify/labeling.py` module (batched LLM call, `graphify label` subcommand). This is a new capability, not a determinism fix — excluded from the determinism lot (F-0820-0827). Reopen in the extractors+platforms lot. |
