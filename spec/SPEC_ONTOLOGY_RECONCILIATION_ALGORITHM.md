...
### 2a-bis. Structural tier (`tier: "structural"`, capability-gated, LOWEST confidence)

A third deterministic tier answering the case the lexical tiers structurally cannot: **two entities that are the same real thing but whose strings do not look alike.** It is **capability-gated** (`structural`, default **OFF**), runs **last**, only over pairs no lexical tier already emitted, and scores in **0.50–0.65 — strictly below the fuzzy floor (0.70)**, so it can never outrank a lexical match. It emits `status: "candidate"` `accept_match` **proposals only**; reconciliation never applies anything (there is no auto-apply path in `src/`).

**Signals.** Shared-neighbour Jaccard over *informative* neighbours, directed relation-type profile Jaccard (`out:<type>`/`in:<type>`), and shared **registry** provenance. Every candidate carries a `structural_basis` (the shared neighbour ids, shared relation types, shared registry refs) and a `score_breakdown`, so a human sees exactly what the proposal rests on.

**Blocking.** Candidate pairs come from an inverted *neighbour → nodes* index, so only nodes sharing an informative neighbour are ever scored. Because a neighbour is informative only when its degree ≤ `hubDegreeMax`, each block holds ≤ `hubDegreeMax` nodes. Measured on the ACLP bundle (38 853 nodes / 13 921 relations): **38 669 blocked pairs built in 147 ms**, versus **7.55 × 10⁸** pairs for the O(n²) scan the lexical tiers used **as of 2026-07-26**.

**Scale-limit clause — HISTORICAL, lifted by `78179b43` (A2-1).** The sentence above originally continued: *"…the O(n²) scan the lexical tiers **still use** (that scan does **not** terminate in practice at 38 k nodes — a known limitation, **unchanged by this work**)"*. Both clauses were accurate when written and are **no longer true**. `78179b43` replaced that scan with lossless inverted-index blocking in **both** lexical tiers: on the same ACLP bundle the lexical pass now completes in **~24 s**, where the pre-A2-1 baseline did not finish inside a 10-minute bound. The blocking is an *equivalence*, not an approximation — output is byte-identical to a naive-scan oracle — so no recall was traded for the speed. The **38 669 pairs / 147 ms** figure above is unaffected: it measures the structural tier's own blocking and remains current.

> A declaration of a LIMIT must carry the date and the commit that establishes it. Otherwise it outlives its own correction and the document starts lying by sequence rather than by error — which is exactly what happened to the two clauses above.

**The measured problem this tier must survive.** Raw shared-neighbour similarity finds **siblings, not duplicates**. Unguarded, it emitted **18 candidates on the mystery pack** and **119 on ACLP**, and every pair inspected was a false positive: *Father Brown ↔ Flambeau* (0.65 — two protagonists of the same stories), *M. Formery ↔ M. Gournay-Martin*, *Google Sheets ↔ Google Drive* (1.00 neighbour Jaccard — two tools supporting the same five processes), *CATIA V5 ↔ ENOVIA*. Two entities that play the *same role in the same context* are maximally structurally similar and are precisely **not** the same entity.

**False-positive guards** (each pinned by a test in `tests/ontology-reconciliation-structural.test.ts`):

- **(S1) Evidential mass.** Both nodes need ≥ `minDegree` (3) informative neighbours and ≥ `minSharedNeighbours` (3) shared ones. A single shared parent gives neighbour-Jaccard 1.0 while naming two siblings — this kills that class at the root.
- **(S2) Hub discounting.** A neighbour with degree > `hubDegreeMax` (50) connects to everything and carries no identifying information; it is removed from **both** neighbour sets before any overlap is computed. The structural analogue of the lexical generic-noun stop-list.
- **(S3) Lexical veto.** `differentEntityReason` runs on the pair and **vetoes** it. Structure may never re-admit a pair the lexical guards confidently rejected (opposite-gender titles, address divergence, place re-qualification).
- **(S4) Serial divergence.** Two labels bearing **disjoint numeric identifiers** are distinct members of a series. **This is the explicit guard against reproducing the known `HC-14 → COMPTON:C-15` fuzzy false positive (0.45) in structural form**, and it is tested by name. On ACLP it rejects 6 734 ordinal-series pairs and 968 divergent-serial pairs.
- **(S5) Direct-edge veto.** A relation between the two nodes is the *corpus itself* asserting they are distinct — an entity does not `oppose`, `assist` or act as `accomplice_of` itself. Only identity relations (`alias_of`, `same_as`, `duplicate_of`, `canonical_of`) are compatible with the pair being one entity.
- **(S6) Container discounting.** Neighbours of a container type (`Work`, `ChapterOrStory`, `Scene`, `Section`, `Saga`) are dropped: sharing a chapter means two entities were *mentioned together*, which every pair of protagonists does. Container types are also excluded from the tier as subjects.
- **(S7) Same-source sibling veto.** Two nodes extracted from the **same document** were separated *by that extraction pass*, so their structural resemblance is co-occurrence. Only entities seen in **different** sources can be two records of one thing. A shared **registry record** overrides the veto — that is positive identity evidence, unlike mere co-mention.

**Measured result.** With the guard suite, both corpora emit **0 structural candidates**: mystery **18 → 0**, ACLP **119 → 0**, i.e. every measured false positive is eliminated and none is replaced by a new one. Enabling the tier is **strictly additive** — verified on mystery that all 31 pre-existing candidates are byte-identical and none is dropped.

**Recall is therefore unproven on the available corpora, by construction of those corpora**: in ACLP only **1 of 29 503** blocked same-type pairs is cross-source (its relation graph is entirely within-document), so there is almost nothing for a cross-source structural match to find. The tier's value is realised on a graph carrying **cross-document structure**; a positive-control test proves it fires on a genuine cross-source duplicate, so the quiet behaviour is correctness, not vacuity. The honest summary: **shared-neighbour similarity is a sound *blocking* signal and a poor *matching* signal**, and this tier is calibrated accordingly — precision-first, default OFF.
