...
3. **De-orphan (`deOrphanByContainer`).** Links each entity that is NOT in the **giant connected component** (degree-0 orphans AND members of tiny disconnected islands) to a container that is ITSELF in the giant component, through a derived `appears_in` edge (`derived:true`, `derivation_method:"deorphan:giant-component"`, `confidence:"INFERRED"`). Resolution per orphan: the FINEST in-giant container (ChapterOrStory/Scene/Section sharing provenance via `source_file` or path slug), else the Work. A provenance-sharing container that is itself isolated is **rejected** (it would merely extend the island), so no orphan is ever linked into a fresh **2-node island** — orphans JOIN the giant. The Work is the root anchor ("necessarily linked to many") and is always a valid target so a fresh edge-less extraction still seeds; finer containers are offered only when in-giant. Because only out-of-giant nodes are targeted, an entity that already reaches its Work via a chapter is left untouched (no redundant entity→Work edge, no star inflation). De-orphan never fabricates inter-entity or cross-saga edges: a legitimately separate saga remains its own component rather than being force-fused into the giant. Idempotent: respects pre-existing `appears_in`, never double-adds, never links a container node into itself. (`joinGiantComponent:false` opts back into the legacy degree-0-only behavior.) **Fix history:** the original 0.x de-orphan only saw degree-0 nodes and would link an orphan to an isolated finest-container, producing implausible 2-node islands (TRACKED #3, e.g. a ForensicMethod alone, or an entity-pair disconnected from the graph); the giant-component join above corrects this. The residual low-edge-density that leaves dense chapter hubs (entities carrying only `appears_in`) is the multi-model re-indexation cause, tracked separately (TRACKED #5).
