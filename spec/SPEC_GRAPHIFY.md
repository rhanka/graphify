...
3. **De-orphan (`deOrphanByContainer`).** Links each degree-0 entity node into the graph — but ONLY through an edge the corpus actually supports. Two grounds are admitted, and nothing else: **containment** (a ChapterOrStory/Scene/Section, else the Work, whose provenance — `source_file`, else path slug — matches the orphan's) emitted as `appears_in`, and **co-provenance** (a node that is already connected and shares the orphan's EXACT `source_file`, i.e. the same document) emitted as `related_to`. Among grounded anchors the one that lands the orphan in the largest reachable component wins, so de-orphaning GROWS the connected component: container-in-giant (`deorphan:giant-component`) → co-provenance peer in the giant (`deorphan:co-provenance-peer`) → container outside the giant (`deorphan:container-offgiant`, still true containment) → co-provenance peer outside the giant → **nothing**. An orphan with neither ground **stays orphaned** and is reported in `unattachable` — de-orphan never invents an edge to make a number look better. **Anti-star:** co-provenance attachments for one document are SPREAD over its top-ranked peers (√-rule: `ceil(sqrt(n))` distinct anchors, ranked component-size ↓ / degree ↓ / id ↑; `maxAnchorFanOut` spreads further), because every peer of that document is an equally valid ground and funnelling them all onto the densest one manufactures a hub-spoke star that describes the algorithm, not the corpus. Containment is deliberately NOT spread — a chapter really does contain all its entities. Exactly ONE anchor per orphan; deterministic (orphans processed in sorted id order), O(n log n), no LLM, idempotent. `DeOrphanResult` reports `byMethod`, `maxAnchorFanOut`, `containerOrphans` and `unattachable`. Set `preferGiantComponent:false` for the legacy strict-finest behavior (`derivation_method:"deorphan:finest-container"`), `joinGiantViaHub:false` to disable peer attachment. **Measured (ACLP, 47 762 nodes / 24 963 edges, 20 389 orphans).** The previous policy's global-hub fallback wired every otherwise-unattachable orphan to ONE node: 20 389 derived edges on a single anchor whose degree went 39 → 20 417, of which 19 542 had no provenance relation at all. The policy above attaches 16 516 orphans over 5 624 distinct anchors with a worst fan-out of 35, leaves the graph's densest node at degree 40, cuts components 24 504 → 7 988 and isolates 20 400 → 3 880, creates zero new 2-node islands (1 379 → 1 153), and reports the remaining **3 873 orphans as `unattachable`** — they are the sole nodes of documents from which nothing else was extracted, so no honest edge exists for them. On the mystery-sagas bundle (1 983 nodes) the outcome is unchanged from the previous policy: 61 `deorphan:giant-component` attachments over 10 anchors, one component, 0 unattachable. **Honest residual:** de-orphan is a topology guardrail, not a density fix — on the public mystery-sagas corpus ~64.8% of entities carry ONLY an `appears_in` edge, which produces implausibly thin hub-spoke stars (e.g. a Father Brown chapter with ~184 degree-1 satellites) and a multi-work Thorndyke island that no container-linking strategy can merge into the giant without cross-work edges. That deeper low-density cause is the separate re-index work (TRACKED #5), not de-orphan.
