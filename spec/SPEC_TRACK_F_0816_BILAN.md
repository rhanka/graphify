...
| 11c | `b6127aa` sub | #956 | OpenCode + Codex semantic-fragment validation (#825) in skill merges | `__main__.py`, skill merge logic | `src/semantic-fragment-validation.ts`, `src/skill-runtime.ts`, `src/skills/`, skill-fragment tests | must-port (M) | F-0816-M4 | no | Skill merge hygiene — fragment correctness for untrusted LLM JSON. Ported as `semantic-fragment-validation.ts` after the release correction clarified upstream `semantic_cleanup.py` semantics. |
...
| 11h | `b6127aa` sub | #956 | semantic_cleanup module (semantic-fragment validation/sanitization, not stale-node pruning) | new `semantic_cleanup.py` (+319) | `src/semantic-fragment-validation.ts`, `src/skill-runtime.ts`, `src/skills/` | must-port (M) | F-0816-M4 | no | Corrected during release 0.9.7: upstream `semantic_cleanup.py` validates and sanitizes untrusted skill-LLM semantic fragments. The TS port lands as `semantic-fragment-validation.ts` to avoid collision with the local `src/semantic-cleanup.ts` stale-node pruner shipped in F-0816-M5. |
