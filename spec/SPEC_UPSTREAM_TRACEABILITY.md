...
| Safi Python Graphify | remote `v7` branch | `0c29b2cb88c6274d889ca7c33a684ce103808715` | `0c29b2cb88c6274d889ca7c33a684ce103808715` | `0.7.5`..`0.7.10` plus post-tag fixes | `partial` | New drift observed on 2026-05-08. Remote tag `v0.7.10` is `ef1050b0e4134df0bd59956b0f900dc3c83e8184`; `upstream/v7` contains four additional commits after that tag. Post-tag `buildMerge` direction preservation, top-level uninstall, missing-skill repair warning, atomic skill writes, SQL ALTER/schema-qualified extraction, CommonJS `require()` import extraction, TS/TSX declaration/call-surface extraction, Markdown/MDX/Quarto structural extraction, and selected no-Python language fallbacks are ported; remaining non-parser rows stay under active audit. |
...
| `v0.7.3` | `d40e274` | `graphify extract` headless semantic extraction for CI, backend selection, `--no-cluster`, `--out`. | `covered` / `intentional-delta` | Functional lot: parser robustness, export surface, and headless extraction. Public coverage is provided by `tests/cli-runtime.test.ts` (`supports code-only headless extract via the public CLI`), `tests/cli-runtime.test.ts` (`supports extract --no-cluster for raw merged extraction output`), `tests/cli-runtime.test.ts` (`requires provided semantic extraction for non-code headless corpora`), and `tests/cli-runtime.test.ts` (`supports docs-only headless extract when semantic JSON is provided`). The intentional delta is backend execution: the TypeScript CLI does not own Anthropic/Kimi backend selection or direct LLM execution. Instead it accepts precomputed semantic JSON and leaves live semantic extraction to the assistant/runtime pipeline, preserving the TypeScript-only, no-Python product line. |
