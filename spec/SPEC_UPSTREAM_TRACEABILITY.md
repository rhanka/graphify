...
| Ref | Commit | Upstream change | TypeScript status | Required follow-up |
| --- | --- | --- | --- | --- |
| `v0.6.0` | `17fb524` | SQL AST extraction, YAML indexing, roll-up fixes from the `0.5.6` / `0.5.7` line. | `needs-review` | Audit SQL parser scope, YAML indexing behavior, and any already-shipped equivalent fixes before claiming parity. |
| `v0.6.1` | `2dc759a` | Exact gitignore semantics, anchored parent-relative patterns, hermetic non-VCS scan boundaries. | `needs-review` | Compare upstream ignore semantics with the current `.graphifyignore` engine and input-scope behavior; add regression tests. |
| `v0.6.2` | `be83a8c` | Exact-match query ordering, content-hash-aware `update`, R support, shebang-based shell detection, Kimi/license cleanup. | `needs-review` / `n/a` | Audit query ranking, `update` behavior, and language-surface deltas; keep Kimi-specific packaging notes as `n/a` where they do not map. |
| `v0.6.3` | `a4149df` | Preserve semantic nodes on incremental rebuild, detached hooks, common-name suppression, `cluster-only` crash guard. | `needs-review` | Compare update/watch/hook/runtime behavior and prove parity with tests. |
| `v0.6.4` | `a61b25c` | Cross-platform Codex hook check on Windows. | `needs-review` | Audit the current Codex hook installer/runtime against the upstream shell-agnostic contract. |
| `v0.6.5` | `d40e1c0` | Codex Windows hook, Kotlin call edges, `update --force`, community checkbox UI. | `needs-review` | Audit hook portability, Kotlin extraction, guarded force-update behavior, and HTML control parity. |
| `v0.6.6` | `517f3c8` | Pi-agent installer support, stale wiki clearing, Windows-safe wiki filenames. | `needs-review` / `deferred` | Decide whether Pi belongs in the TypeScript platform matrix; audit wiki regeneration and filename sanitization either way. |
| `v0.6.7` | `e484282` | `graphify tree`, token-aware chunking, MCP context filters, dynamic `import()`, safe semantic-cache file checks. | `needs-review` | Audit the tree/export surface, chunking policy, context filtering, JS import extraction, and directory-safe cache handling. |
| `v0.6.8` | `d753413` | `.graphifyignore` negation patterns, Antigravity frontmatter, Gemini/Codex hook fixes, thin-community omission. | `needs-review` | Audit ignore negation semantics, platform hook installers, and report community filtering. |
| `v0.6.9` | `f81e3bc` | Slash-normalized `source_file`, cohesion re-splitting, VS Code Copilot instruction contract, `GRAPHIFY_OUT`, Antigravity reinstall. | `needs-review` / `intentional-delta` | Audit portability/report clustering changes; explicitly decide whether `GRAPHIFY_OUT` fits the `.graphify/` contract or stays a deliberate delta. |
| `v0.7.0` | `f81e3bc` | Merge driver for `graph.json`, deterministic community IDs, content-only cache hashing, freshness signal, mixed code/doc change handling. | `needs-review` | Audit multi-dev workflow support against the current `.graphify` lifecycle and portable artifact model. |
| `v0.7.1` | `f81e3bc` | Obsidian tag sanitization, extended `tsconfig` alias resolution, Svelte template dynamic imports, recursion safety. | `needs-review` | Audit wiki/export sanitization, TS config resolution, Svelte import extraction, and deep-AST robustness. |
| `v0.7.2` | `f81e3bc` | Fortran support, export CLI subcommands, skill-size reduction, large-graph aggregation. | `needs-review` / `deferred` | Audit language and export surfaces; treat skill-size reduction as non-blocking unless it changes runtime behavior or install limits. |
| `v0.7.3` | `f81e3bc` | `graphify extract` headless semantic extraction for CI, backend selection, `--no-cluster`, `--out`. | `needs-review` / `intentional-delta` | Compare the upstream headless extraction flow with the current TypeScript assistant/runtime contract before deciding parity or a documented delta. |
| `v0.7.4` | `f81e3bc` | JSONC `tsconfig` parsing and aliased Svelte dynamic-import fixes. | `needs-review` | Close parser/import parity and use this row as the final release gate before publishing `0.7.4`. |
