...
| `v0.6.5` | `d40e1c0` | Codex Windows hook, Kotlin call edges, `update --force`, community checkbox UI. | `partial` | Functional lot: incremental rebuild reliability, hooks, and platform surface. `update --force` is now covered by `tests/export-json.test.ts` and `tests/cli-runtime.test.ts`: guarded rebuilds fail cleanly when `graph.json` would shrink, and `--force` explicitly overrides that guard. Hook portability, Kotlin extraction, and HTML checkbox controls remain open. |
