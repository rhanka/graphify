...
| Ref | Commit | Upstream change | TypeScript status | Required follow-up |
| --- | --- | --- | --- | --- |
| `v0.6.0` | `17fb524` | SQL AST extraction, YAML indexing, roll-up fixes from the `0.5.6` / `0.5.7` line. | `needs-review` | Functional lot: structured inputs, query precision, and inventory semantics. Audit SQL parser scope, YAML indexing behavior, and any already-shipped equivalent fixes before claiming parity. |
...
| `v0.6.2` | `be83a8c` | Exact-match query ordering, content-hash-aware `update`, R support, shebang-based shell detection, Kimi/license cleanup. | `needs-review` / `n/a` | Functional lot: structured inputs, query precision, and inventory semantics. Audit query ranking, `update` behavior, and language-surface deltas; keep Kimi-specific packaging notes as `n/a` where they do not map. |
