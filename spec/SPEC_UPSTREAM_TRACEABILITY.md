...
| `v0.7.2` | `b6ffdbb` | Fortran support, export CLI subcommands, skill-size reduction, large-graph aggregation. | `needs-review` / `deferred` | Functional lot: parser robustness, export surface, and headless extraction. Audit language and export surfaces against the effective `upstream/v7` continuation; treat skill-size reduction as non-blocking unless it changes runtime behavior or install limits. |
