...
**Auteur / modèle :** OpenAI Codex (`GPT-5`, identité d'exécution de cette session)
...
| Suppression | Utopia conserve et peut réactiver le contenu (`Observed in code — primary source`, [migration 0022][u-delete-migration]). NOTRE tombstone est terminal et cascaded (v2 §5.6, lignes 614–629; lot L6, lignes 1160–1163). | Sémantique de deletion : **NON-ADOPTABLE**. |
...
| Profil inspiré des packs comme donnée de projection | **ADOPTABLE** | `Observed in code — primary source` : packs versionnables/embarqués ([ontology_packs.rs, lignes 34–80][u-packs-code]). `OUR` : bridges/projections hors canon (v2 D1, ligne 20; §5.8, lignes 793–839). | Profil deployment-owned, sans champ canonique `(inference)`. |
...
| Façade MCP read-only avec auth par appel | **ADOPTABLE comme pattern external-host** | `Observed in code — primary source` : [mcp.rs, lignes 35–86 et 135–205][u-mcp]. `OUR` : authorization/revalidation (v2 §5.2, lignes 261–353; §5.7, lignes 789–791). | Toute sortie repasse par le store canonique `(inference)`. |
...
[u-head-api]: https://api.github.com/repos/deeplethe/utopia/commits/dev
